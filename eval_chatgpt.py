...
class Evaluator:
        ...
        while True:
            try:
                completion = openai.chat.completions.create(
                    model=self.model,
                    messages=[
                        {"role": "system", "content": f"Now you should choose the correct answer regarding the question and options. You should ONLY return the correct answer by following this template: {self.answer_template}\n\n You **MUST** follow the template to answer!!! Other format answers will be strongly rejected!"},
    ...
    def evaluate(self):
        for idx, instance in enumerate(self.benchmark):
            if idx <= -1:
                print("idx: ", idx)
                with open("/Users/mellen/Desktop/MeRE-ms/project/eval_results/gpt-4-0125-preview_eval___FCO775___status.json", mode='r') as g:
...
if __name__ == '__main__':
    ...
    evaluator = Evaluator(benchmark_json=benchmark, model_name='gpt-3.5-turbo', bench_name="__FCO775_temp0.9_")
    evaluator.evaluate()